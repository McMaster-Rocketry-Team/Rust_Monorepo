...
///
/// Discrete-time process model with known vertical acceleration input `u` (m s⁻²):
///     altitudeₖ₊₁ = altitudeₖ + vertical_speedₖ · dt + 0.5 · u · dt² + w₁
///     vertical_speedₖ₊₁ = vertical_speedₖ           + u · dt         + w₂
///
/// Discrete form:
///     xₖ₊₁ = F · xₖ + B · u + w,   w ~ 𝒩(0,Q)
///     zₖ   = H · xₖ + v,   v ~ 𝒩(0,R)
///
/// F = ⎡1  dt⎤ ,  H = ⎡1  0⎤ ,  B = ⎡0.5·dt²⎤
///     ⎣0  1 ⎦                      ⎣  dt   ⎦
#[derive(Debug, Clone)]
pub struct AltitudeKF {
    ...
    /// Measurement matrix
    h: SMatrix<f32, 1, 2>,
    /// Control-input vector for vertical acceleration (B)
    b: SVector<f32, 2>,
...
impl AltitudeKF {
    ...
    pub fn new(initial_altitude: f32, measurement_variance: f32) -> Self {
        ...
        // Control-input vector for vertical acceleration
        let b = SVector::<f32, 2>::new(0.5 * DT.powi(2), DT);
    ...
    /// Predict state dt seconds ahead with a known vertical acceleration input (m s⁻²)
    pub fn predict(&mut self, vertical_acc: f32) {
        // x̂₋ = F x̂ + B u
        self.x = self.f * self.x + self.b * vertical_acc;
