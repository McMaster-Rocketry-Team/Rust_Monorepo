...
impl MessageRow {
    ...
    /// Payload readings arrive as an `Option` from the message's accessors,
    /// which decode the CAN `0xFFFF` = unavailable sentinel at that boundary.
    /// Nothing here touches the raw fields, so a rail sitting at a genuine 0mA
    /// and a rail EPM could not read stay two different things all the way to
    /// the screen.
    fn format_rail_mv(mv: Option<u16>) -> StyledString {
        match mv {
    ...
    fn format_rail_ma(ma: Option<u16>) -> StyledString {
        match ma {
    ...
    fn format_steps(steps: Option<u16>) -> StyledString {
        match steps {
