...
/// USB serial number advertised by the VLF5 (`usb_handler.rs`).
const VLF5_SERIAL: &str = "4206980085";
/// Bulk-IN endpoint address (EP 1 IN).
const EP_IN: u8 = 0x81;
/// Vendor interface number.
const INTERFACE: u8 = 0;

/// Find the VLF5 on the USB bus and claim its interface.
fn find_and_open() -> Result<DeviceHandle<Context>> {
    ...
    for device in ctx.devices().context("listing USB devices")?.iter() {
        if device_serial(&device).ok().flatten().as_deref() == Some(VLF5_SERIAL) {
    ...
    bail!("VLF5 not found over USB. Is it plugged in via USB-C and powered on?")
}

fn device_serial(device: &Device<Context>) -> Result<Option<String>> {
    let desc = device.device_descriptor()?;
    let Some(idx) = desc.serial_number_string_index() else {
        return Ok(None);
    };
    let handle = device.open()?;
    let langs = handle.read_languages(Duration::from_secs(1))?;
    let Some(lang) = langs.first().copied() else {
        return Ok(None);
    };
    Ok(Some(handle.read_string_descriptor(
        lang,
        idx,
